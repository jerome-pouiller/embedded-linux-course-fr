...
\end{lstlisting} %$
    \item   Pas   sécurisé,   attention   a   votre   mot   de   passe
      \note[item]{faire une  démonstration avec telnetd  et tcpdump -i
        lo -A  port telnet (il  faut regarder le dernier  caractère de
...
\end{lstlisting} %$
  \note[item]{TP: Just do it, surtout wget}
...
\begin{frame}[fragile=singleslide]{Notre premier programme}{\file{hello.c}}
  \embedfile{hello/hello.c}
  \lstinputlisting[language=c,lastline=15]{hello/hello.c}
\end{frame}

\begin{frame}[fragile=singleslide]{Notre premier programme}{\file{hello.c}}
  \lstinputlisting[language=c,firstline=16]{hello/hello.c}
\end{frame}

\begin{frame}[fragile=singleslide]{Notre premier programme}{\file{hello.h}}
  \embedfile{hello/hello.h}
  \lstinputlisting[language=c]{hello/hello.h}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Créer un projet sans autotools}
  ...
  \lstinputlisting{hello/Makefile.1}
  Testons:
\begin{lstlisting}
...
\begin{frame}[fragile=singleslide]{Créer un projet sans autotools}
  ...
  \lstinputlisting{hello/Makefile.2}
  Testons:
\begin{lstlisting}
host$ cd build
...
\begin{frame}[fragile=singleslide]{Créer un projet sans autotools}
  ...
  ainsi des morceaux de Makefile que l'on inclut. Il ne faut pas oublier
  d'ajouter la dépendance entre \cmd{hello.d} et les dépendances de
  \cmd{hello.c}
  \lstinputlisting{hello/Makefile.3}
...
\begin{frame}[fragile=singleslide]{Créer un projet sans autotools}
  ...
  \lstinputlisting{hello/Makefile.4}
\end{frame}

\begin{frame}[fragile=singleslide]{Créer un projet sans autotools}
  Nous pouvons  ajouter des alias  pour nous aider dans  les commandes
  complexes 
  \lstinputlisting[firstline=16]{hello/Makefile.5}
  \note[item]{Ajouter des \#ifdef CONFIG pour faire dans Kconfig}
\end{frame}
